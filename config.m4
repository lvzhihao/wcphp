...
if test "$PHP_WORDCHECKR" != "no"; then
  ...
  dnl if test -r $PHP_WORDCHECKR/$SEARCH_FOR; then # path given as parameter
  dnl   WORDCHECKR_DIR=$PHP_WORDCHECKR
  dnl else # search default path list
  dnl   AC_MSG_CHECKING([for wordcheck files in default path])
  dnl   for i in $SEARCH_PATH ; do
  dnl     if test -r $i/$SEARCH_FOR; then
  dnl       WORDCHECKR_DIR=$i
  dnl       AC_MSG_RESULT(found in $i)
  dnl     fi
  dnl   done
  dnl fi
  dnl
  dnl if test -z "$WORDCHECKR_DIR"; then
  dnl   AC_MSG_RESULT([not found])
  dnl   AC_MSG_ERROR([Please reinstall the wordcheck distribution])
  dnl fi

  dnl # --with-wordcheck -> add include path
  dnl PHP_ADD_INCLUDE($WORDCHECKR_DIR/include)
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $WORDCHECKR_DIR/lib, WORDCHECKR_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_WORDCHECKRLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong wordcheck lib version or lib not found])
  dnl ],[
  dnl   -L$WORDCHECKR_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(WORDCHECKR_SHARED_LIBADD)
