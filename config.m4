...
dnl PHP_ARG_WITH(wordcheck, for wordcheck support,
dnl Make sure that the comment is aligned:
dnl [  --with-wordcheck             Include wordcheck support])

dnl Otherwise use enable:

PHP_ARG_ENABLE(wordcheck, whether to enable wordcheck support,
Make sure that the comment is aligned:
[  --enable-wordcheck           Enable wordcheck support])
...
if test "$PHP_WORDCHECK" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $WORDCHECK_DIR/$PHP_LIBDIR, WORDCHECK_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_WORDCHECKLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong wordcheck lib version or lib not found])
  dnl ],[
  dnl   -L$WORDCHECK_DIR/$PHP_LIBDIR -lm
  dnl ])
  dnl
  ...
  PHP_NEW_EXTENSION(wordcheck, wordcheck.c, $ext_shared,, -DZEND_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
